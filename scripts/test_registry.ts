...

{
  ...
  try {
    ...
    registry.upsertPool({
      ...
      block: 12,
      tx: "0xcafebabe",
      metadata: JSON.stringify({ fee: "3000", tickSpacing: 60 }),
    });

    const directUpsertedPool = registry.getPool("0xCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC");
    assert.deepEqual(
      directUpsertedPool?.tokens,
      [
        "0x1111111111111111111111111111111111111111",
        "0x2222222222222222222222222222222222222222",
      ],
      "direct upsertPool should normalize JSON-string token lists instead of silently storing an empty token set",
    );
    assert.deepEqual(
      directUpsertedPool?.metadata,
      { fee: "3000", tickSpacing: 60 },
      "direct upsertPool should normalize JSON-string metadata into the canonical object form",
    );

    const activeBeforeDisable = registry.getActivePoolsMeta();
    assert.equal(activeBeforeDisable.length, 3, "active meta cache should contain all inserted pools");
    assert.equal(
    ...
    assert.equal(activeAfterEnable.length, 3, "enablePool should repopulate the active metadata view");
    ...
    assert.equal(
      registry.getPoolMeta("0xBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBB")?.status,
      "removed",
      "batchRemovePools should soft-remove pools while preserving their metadata row",
    );
    assert.equal(
      registry.getPoolMeta("0xBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBB")?.removed_block,
      48,
      "soft removals should persist the first removal block so rollback can reason about them",
    );
    assert.equal(
      registry.getPoolMeta("0xCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC")?.removed_block,
      55,
      "later soft removals should retain their own removal block",
    );
    ...
    const nonRegressedStateRow = registry.db
      .statement(
        "test:getPoolStateRowAfterStaleWrite",
        "SELECT address, last_updated_block, state_data FROM pool_state WHERE address = ?",
      )
      .get("0xaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa");
    assert.equal(
      nonRegressedStateRow.last_updated_block,
      50,
      "batchUpdateStates should ignore stale writes that arrive after a newer persisted state",
    );
    assert.match(
      String(nonRegressedStateRow.state_data),
      /"reserve0":"1000"/,
      "batchUpdateStates should preserve the newest payload when an older block arrives later",
    );
    ...
    assert.equal(
      registry.getCheckpoint("UNISWAP_V2")?.last_block_hash ?? null,
      null,
      "rollback should clear checkpoint hashes when the checkpoint block is rewound",
    );
    assert.equal(rollbackResult.poolsReactivated, 1, "rollback should reactivate pools removed at or after the reorg block");
    assert.equal(
      registry.getPoolMeta("0xBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBB")?.status,
      "removed",
      "rollback should preserve soft removals that happened before the reorg block",
    );
    assert.equal(
      registry.getPoolMeta("0xCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC")?.status,
      "active",
      "rollback should reactivate pools whose removal happened inside the reverted block range",
    );
    assert.equal(
      registry.getPoolMeta("0xCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC")?.removed_block ?? null,
      null,
...

{
  ...
  assert.deepEqual(
    parsePoolMetadata(JSON.stringify(JSON.stringify({ fee: "3000", tickSpacing: 60 }))),
    { fee: "3000", tickSpacing: 60 },
    "metadata parsing should unwrap double-encoded JSON produced by inconsistent callers",
  );
  assert.deepEqual(
    parsePoolMetadata("[]"),
    {},
    "metadata parsing should reject array-shaped payloads and fall back to an object",
  );
}
