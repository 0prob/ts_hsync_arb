```bash
graphify query Audit\ runtime\ configuration\ and\ operator\ scripts\ across\ package.json\,\ .env.example\,\ src/config/index.ts\,\ runner.ts\,\ tune_performance.ts\,\ performance\ cache\ files\,\ metrics\ configuration\,\ HyperSync/RPC\ env\ vars\,\ live-mode\ keys\,\ and\ test\ entrypoints.\ Find\ hard-coded\ defaults\,\ undocumented\ env\ vars\,\ unsafe\ fallbacks\,\ script\ drift\,\ and\ misconfigurations\ that\ could\ make\ a\ production\ run\ differ\ from\ tested\ behavior. --budget 3500
```
